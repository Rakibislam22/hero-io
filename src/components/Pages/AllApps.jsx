...
const trendDataPromise = fetch('/trendingData.json').then(res => res.json());

const AllApps = () => {
       const allData = use(allDataPromise);
       const trendData = use(trendDataPromise);
    return (
        <div className='max-w-7xl mx-auto px-6 py-12'>
            ...
            <div className='flex  justify-between items-center'>
                <div>
                    <p className='text-2xl text-[#001931] font-semibold py-5'>({allData.length+trendData.length}) Apps Found</p>
                </div>
            ...
            <div className='grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1  gap-5'>
                    ...
                    {
                        trendData.map(data => <Sapp key={data.id} data={data}></Sapp>)
                    }
                </div>
        </div>
    );
